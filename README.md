...
- **Edge protection and security headers.** Route protection and hardening headers (`X-Frame-Options`, `Strict-Transport-Security`, etc.) run in a single Next.js middleware (`proxy.ts`).
...
- **Performance.** The home page is fully server-rendered with no client spinner or layout shift. The browse page genre list is cached with `unstable_cache` (1h TTL) to avoid a full table scan on every request. Cloudinary images are cached 24h via `minimumCacheTTL`; above-the-fold images use the `priority` prop for faster LCP.
